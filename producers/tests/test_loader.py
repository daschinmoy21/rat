...
def write_adapter(root, name, body="app.add_source('NAME', poll=lambda since: [])"):
    (root / name / "adapter.py").write_text(body.replace("NAME", name))


def test_broken_schema_leaves_no_half_loaded_plugin(tmp_path):
    write_plugin(tmp_path, "bad", "events.bad")
    write_adapter(tmp_path, "bad")
    (tmp_path / "bad" / "schema.json").write_text("{invalid json")
    app = App()
